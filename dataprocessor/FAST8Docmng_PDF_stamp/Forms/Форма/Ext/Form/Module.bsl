...
Процедура ТестНаСервере()
	...
	Запрос.УстановитьПараметр("ВладелецФайла", Объект.Входящий);
...
&НаСервере
Процедура ИнсталлНаСервере()
	ЗапуститьПриложение("pip install reportlab", "F:\python_docmng\", Истина);
	ЗапуститьПриложение("pip install PyPDF2", "F:\python_docmng\", Истина);
КонецПроцедуры

&НаКлиенте
Процедура Инсталл(Команда)
	ИнсталлНаСервере();
КонецПроцедуры
